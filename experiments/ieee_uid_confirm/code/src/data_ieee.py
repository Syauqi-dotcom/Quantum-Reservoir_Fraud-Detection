...
_DERIVED = ["email_risk", "hour", "dow", "fraud_type", "entity", "D1n"]
...
def load(cfg: dict, seed: int) -> pd.DataFrame:
    ...
    # UID trick from the IEEE-CIS 1st-place solution: D1 = days-since-account-
    # seen, so it grows by ~1 every day for the SAME account and can't be used
    # to group directly. day-of-transaction minus D1 cancels that drift and is
    # (approximately) constant for the same underlying client -> a much less
    # noisy entity key than card1+addr1 alone when combined with them below
    # (config.data.entity_keys.E2). No leakage: D1 and TransactionDT are both
    # already known at the time of the transaction itself.
    day = (df["TransactionDT"] // 86400).astype(np.int64)
    d1 = pd.to_numeric(df.get("D1"), errors="coerce").fillna(0.0)
    df["D1n"] = (day - d1).round().astype(np.int64)
